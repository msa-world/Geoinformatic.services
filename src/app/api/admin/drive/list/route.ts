...
export async function POST(req: Request) {
  ...
  if (!profile || !profile.google_refresh_token) return NextResponse.json({ files: [] });

  try {
    const tokenResp: any = await refreshAccessToken(profile.google_refresh_token);
    const accessToken = tokenResp.access_token;
    const driveRes = await fetch("https://www.googleapis.com/drive/v3/files?pageSize=100&q=trashed%3Dfalse&fields=files(id,name,mimeType,modifiedTime,size,webViewLink,webContentLink)", {
      headers: { Authorization: `Bearer ${accessToken}` },
    });
    if (!driveRes.ok) throw new Error("Drive API failed");
