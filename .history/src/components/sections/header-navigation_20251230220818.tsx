...
export default function HeaderNavigation() {
  ...
  // Use dark text on light backgrounds, white text only on dark hero sections before scrolling
  const textColor = (!isScrolled && hasDarkHero) ? "text-white" : "text-gray-900"
  const activeTextColor = (!isScrolled && hasDarkHero) ? "text-white font-bold" : "text-primary font-bold"
  const hoverTextColor = (!isScrolled && hasDarkHero) ? "hover:text-white/80" : "hover:text-gray-700"
  ...
  useEffect(() => {
    const checkUser = async () => {
      try {
        // Use serialized cached getter to avoid multiple simultaneous refresh calls
        ...
        console.error("[MSA] Error fetching user:", error)
      } finally {
        setIsLoading(false)
      }
    }
  ...
  return (
    <motion.header
      ...
      className={cn(
        ...
        (isScrolled || isContactPage) ? "bg-white/80 backdrop-blur-md shadow-sm border-b border-white/20 py-2" : "bg-transparent py-4",
      )}
    >
            ...
            <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
              <SheetTrigger asChild>
                <Button
                  ...
                  className={cn("transition-colors", !isScrolled ? "text-white hover:bg-white/10" : "hover:bg-accent")}
                >
