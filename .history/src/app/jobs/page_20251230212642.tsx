...
export default function LightIndeedJobsPage() {
    ...
    const fetchJobsAndUserStates = async () => {
        setIsLoading(true);
        try {
            // 1. Fetch Jobs from server API to avoid client-side auth refresh storms
            ...
            // 2. Lightweight user check (cached) - avoid further DB queries here
            try {
                const { getUserOnce } = await import('@/lib/supabase/auth-client')
                const authUser = await getUserOnce()
                setUser(authUser)
            } catch (e) {
                // ignore - we'll treat user as unauthenticated client-side
                setUser(null)
            }

            // Not fetching saved/applied lists on mount to avoid extra requests.
